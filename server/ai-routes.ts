...
export function registerAIRoutes(app: Express) {
  ...
  app.post("/api/admin/ai/generate-content", async (req, res) => {
    try {
      ...
      const generatedContent = await generateContent({ subject, examType, topic });
  ...
  app.post("/api/admin/ai/generate-questions", async (req, res) => {
    try {
      ...
      const result = await generateQuestions({ contentTitle, contentBody, subject });
  ...
  app.post("/api/admin/content/save", async (req, res) => {
    try {
      const { title, subject, examType, definition, keyPoints, example, tip, tags, status } = req.body;
